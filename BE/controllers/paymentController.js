...
export const createPaymentUrl = async (req, res) => {
  try {
    ...
    const paymentUrl = await vnpay.buildPaymentUrl({
      vnp_Amount: appointment.price * 100, // VNPay yêu cầu nhân 100 (đơn vị: đồng)
      vnp_IpAddr: req.ip || "127.0.0.1",
      vnp_TxnRef: txnRef,
      vnp_OrderInfo: `Thanh toán lịch khám - ID: ${appointment._id}`,
      vnp_ReturnUrl:
        process.env.VN_PAY_RETURN_URL ||
  ...
  try {
    ...
    if (verify.isSuccess && verify.isVerified) {
      ...
      const failedPayment = await Payment.findOneAndUpdate(
        { txnRef: verify.vnp_TxnRef },
        { status: "failed", responseCode: verify.vnp_ResponseCode },
        { new: true },
      );

      if (failedPayment?.appointmentId) {
        await Appointment.findByIdAndDelete(failedPayment.appointmentId);
        console.log("Đã xoá appointment:", failedPayment.appointmentId);
      }

      console.log(
        "RETURN: THANH TOÁN THẤT BẠI - Mã lỗi:",
