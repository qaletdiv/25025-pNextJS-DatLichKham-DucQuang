...
export const createAppointment = async (req, res) => {
  try {
    const { id } = req.params;
    const existedAppointment = await Appointment.findOne({ medicalForm: id });
    if (existedAppointment) {
      return res
        .status(400)
        .json({ message: "Phiếu khám này đã có lịch hẹn, không thể tạo mới" });
    }
...
export const getDetailAppoitment = async (req, res) => {
  try {
    ...
    const appointment = await Appointment.findById(id)
      .populate("scheduleId", "startTime endTime date doctorId")
      .populate({
        path: "prescription",
        populate: { path: "medicines.pill" },
      });
    if (!appointment) {
